  ...
  const handleExport = () => {
    ...
    const csvContent = exportData.map((item) => {
      const row = [
        ...
        moment(item.createdAt).format("MMMM Do YYYY, h:mm:ss A"),
        item.order_status,
        item.subTotal || item.totalAmount + (item.discountAmount || 0), // Added Subtotal
        item.couponCode || "NONE", // Added Coupon Code
        item.discountAmount || 0, // Added Discount Amount
        item.totalAmount,
        item.paymentDetails.payment_status,
        item.billing_name,
        item.billing_email,
              ...
              <div className="flex justify-between items-center">
                <div>
                  ...
                  <div className="flex items-center gap-4">
                    <p className="text-brand-textMuted0">Order ID: {item.orderId}</p>
                    {/* 🔥 Added a badge for Coupon if applied in the collapsed view */}
              ...
              {expandedOrder === item.orderId && (
                <div className="mt-4">
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
                    ...
                    <div className="border p-4 rounded-md bg-white shadow">
                      <div className="text-lg font-medium text-blue-600">Price Summary</div>
                      {/* 🔥 Pricing Breakdown Added Here */}
                      <div className="ml-1 mt-1 space-y-1">
                         <p className="flex justify-between border-b border-dashed pb-1">
                            <span>Subtotal:</span>
                            <span>{displayINRCurrency(item.subTotal || item.totalAmount + (item.discountAmount || 0))}</span>
                         </p>
                         {item.discountAmount > 0 && (
                            <p className="flex justify-between text-green-600 font-bold border-b border-dashed pb-1">
                                <span>Coupon ({item.couponCode}):</span>
                                <span>- {displayINRCurrency(item.discountAmount)}</span>
                            </p>
                         )}
                         <p className="flex justify-between font-bold text-brand-primary pt-1">
                            <span>Total Paid:</span>
                            <span>{displayINRCurrency(item.totalAmount)}</span>
                         </p>
                      </div>
                    </div>
                  ...
                  <div className="font-bold mt-4 text-right text-xl text-brand-primary">
                    Final Amount: {displayINRCurrency(item.totalAmount)}
                  </div>
                </div>
              )}
            </div>
