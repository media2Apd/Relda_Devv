...
//       <div className="pt-6 space-y-4 md:space-y-0 grid grid-cols-1 md:grid-cols-2 md:gap-4">
...
import "react-toastify/dist/ReactToastify.css";
import SummaryApi from "../common";
import { FaHeart, FaRegHeart, FaTicketAlt, FaCopy } from "react-icons/fa"; // Added FaTicketAlt, FaCopy
import {
  FaStar,
  FaStarHalfAlt,
  FaFacebookF,
...
const ProductDetails = () => {
  ...
  const fetchProductCoupons = async (productId) => {
    try {
        ...
        if(resData.success) {
            setApplicableCoupons(resData.data);
        }
    } catch (err) {
  ...
  // 🔥 NEW: Copy Coupon Code Function
  const handleCopyCoupon = (code) => {
    navigator.clipboard.writeText(code);
    toast.success(`Code ${code} copied!`, { position: "top-center", autoClose: 1500 });
  };
  ...
  const fetchProductDetails = useCallback(async () => {
    ...
    try {
      ...
      // 🔥 Fetch coupons once product data is ready
      if(dataReponse?.data?._id) {
          fetchProductCoupons(dataReponse.data._id);
      }
            ...
            {applicableCoupons.length > 0 && (
                <div className="my-2">
                    <button 
                        ...
                        className="flex items-center gap-2 text-green-600 bg-green-50 border border-green-200 px-3 py-1.5 rounded-lg hover:bg-green-100 transition-all font-bold text-sm"
                    >
                        <FaTicketAlt className="animate-bounce" />
                        {applicableCoupons.length} {applicableCoupons.length === 1 ? "Offer Available" : "Offers Available"}
                    </button>
                </div>
            )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[2000] p-4">
            <div className="bg-white rounded-2xl shadow-2xl max-w-md w-full overflow-hidden animate-in fade-in zoom-in duration-200">
                <div className="bg-brand-primary p-4 text-white flex justify-between items-center shadow-lg">
                    ...
                    <button onClick={() => setShowOffersModal(false)} className="text-xl hover:scale-110 transition-all"><IoClose /></button>
                </div>
                
                <div className="p-4 space-y-4 max-h-[60vh] overflow-y-auto bg-gray-50 custom-scrollbar">
                            ...
                            <div className="flex justify-between items-center">
                                <div className="flex-1">
                                    <div className="flex items-center gap-2 mb-1">
                                        <span className="bg-brand-primary bg-opacity-10 text-brand-primary px-3 py-1 rounded font-mono font-bold text-sm tracking-widest uppercase">
                                            {coupon.code}
                                        </span>
                                        <button 
                                    ...
                                    <p className="text-[10px] text-gray-500 mt-1 uppercase font-bold tracking-tighter">
                                        Min Order: ₹{coupon.minOrderAmount} | Max Discount: ₹{coupon.maxDiscountAmount || "∞"}
                                    </p>
                                </div>
                            </div>
                        </div>
                ...
                <div className="p-4 bg-white border-t text-center">
                    <button 
                        ...
                        className="w-full bg-gray-100 text-gray-700 font-bold py-2 rounded-lg hover:bg-gray-200 transition-all"
                    >
                        Close
                    </button>
                </div>
